...
function normalizeWeeklyItems(source, entries, now = new Date()) {
  ...
  for (const entry of entries) {
    ...
    } catch { /* invalid official entry is ignored */ }
  }
  ...
  for (const match of body.matchAll(/<script[^>]+type=["']application\/ld\+json["'][^>]*>([\s\S]*?)<\/script>/gi)) {
    try {
      ...
      for (const node of nodes) {
        if (['BlogPosting', 'NewsArticle', 'Article'].includes(node['@type'])) {
          entries.push({ title: node.headline || node.name, url: node.url || node.mainEntityOfPage?.['@id'], publishedAt: node.datePublished, summary: node.description })
        }
      }
...
function parseFeed(body) {
  ...
  return blocks.map(block => ({
    ...
    url: value(block, ['link', 'guid']) || block.match(/<link[^>]+href=["']([^"']+)/i)?.[1],
    publishedAt: value(block, ['pubDate', 'published', 'updated']),
    summary: value(block, ['description', 'summary', 'content']),
  }))
}
...
function parseArticles(body, baseUrl) {
  return [...body.matchAll(/<article\b[^>]*>([\s\S]*?)<\/article>/gi)].map(match => {
    ...
    return {
      title: text(block.match(/<h[1-3]\b[^>]*>([\s\S]*?)<\/h[1-3]>/i)?.[1]),
      url: href ? new URL(href, baseUrl).toString() : '',
      publishedAt: block.match(/<time\b[^>]+datetime=["']([^"']+)["']/i)?.[1],
      summary: text(block.match(/<p\b[^>]*>([\s\S]*?)<\/p>/i)?.[1]),
    }
  })
}

async function fetchWeeklySource(source, { fetcher = fetch, now = new Date(), timeoutMs = 10_000, maxBytes = 2_000_000 } = {}) {
  const controller = new AbortController()
  const timer = setTimeout(() => controller.abort(), timeoutMs)
  let response
  try {
    response = await fetcher(source.url, {
      headers: { Accept: 'application/rss+xml, application/atom+xml, text/html;q=0.9', 'User-Agent': 'SiyuWeekly/1.0' },
...
module.exports = { WEEKLY_SOURCES, fetchWeeklySource, normalizeWeeklyItems }
