...
import { topics, type Topic } from '../topics/topic-data'

const states = ['全部', '讨论中', '待整理', '已沉淀'] as const

export function SpacesView({ onOpenTopic }: { onOpenTopic: (topic: Topic) => void }) {
  ...
  return <main className="overview">
    <header className="overview-hero"><div><span className="eyebrow">思考空间</span><h1>所有思考，都有迹可循</h1><p>从最初的问题到最后的结论，保留每一次观点变化。</p></div><button className="primary" onClick={() => onOpenTopic({ ...topics[0], id: 'new-space', title: '一个新的思考空间' })}><Plus size={18}/>新建空间</button></header>
